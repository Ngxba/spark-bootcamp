...
from pyspark.sql.functions import col, upper, length, when, round as spark_round
...
def exercise_2b(spark: SparkSession, product_data: List[tuple]):
    ...
    result = (
        ...
        .withColumn(
            "discount_eligible",
            when((col("price") > 200) & (col("stock") > 50), True).otherwise(False),
        )
    )
...
def exercise_2c(spark: SparkSession, sales_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        sales_data, ["product", "quantity", "unit_price", "discount"]
    )

    # Chain multiple transformations
    result = (
        df.withColumn("gross_total", col("quantity") * col("unit_price"))
...
def exercise_2d(spark: SparkSession, customer_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        customer_data, ["customer_id", "name", "email", "phone", "age"]
    )

    # Transform and clean data
    result = (
        df.withColumn("name_clean", upper(col("name")))
        .withColumn(
            "email_domain",
            when(
                col("email").contains("@"),
...
def exercise_2e(spark: SparkSession, order_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        order_data, ["order_id", "customer_id", "total_amount", "status", "region"]
    )
...
def exercise_2f(spark: SparkSession, inventory_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        inventory_data, ["item_code", "item_name", "cost", "selling_price", "quantity"]
    )

    # Advanced calculations
    result = (
        df.withColumn("profit_per_unit", col("selling_price") - col("cost"))
        .withColumn(
            "profit_margin",
            spark_round(
                (col("selling_price") - col("cost")) / col("selling_price") * 100, 2
            ),
        )
