...
from pyspark.sql.functions import col, coalesce, lit, when, isnan, isnull, count
...
def setup_spark() -> SparkSession:
    ...
    return SparkSession.builder \
        .appName("Exercise4-Solutions") \
        .master("local[*]") \
        .getOrCreate()


def exercise_4a(spark: SparkSession, employees_data: List[tuple], departments_data: List[tuple]):
    """Perform inner join between employees and departments."""
    # Create DataFrames
    employees_df = spark.createDataFrame(employees_data, ["emp_id", "name", "dept_id", "salary"])
...
def exercise_4b(spark: SparkSession, customers_data: List[tuple], orders_data: List[tuple]):
    ...
    orders_df = spark.createDataFrame(orders_data, ["order_id", "customer_id", "order_amount", "order_date"])

    # Left join with aggregation
    result = customers_df.join(orders_df, "customer_id", "left") \
                        .groupBy("customer_id", "customer_name", "email") \
                        .agg(
                            count(orders_df.order_id).alias("order_count"),
                            coalesce(sum(orders_df.order_amount), lit(0.0)).alias("total_spent")
                        ) \
                        .orderBy("customer_id")
...
def exercise_4c(spark: SparkSession, products_data: List[tuple], inventory_data: List[tuple]):
    ...
    inventory_df = spark.createDataFrame(inventory_data, ["product_id", "warehouse", "quantity", "last_updated"])

    # Right join
    result = products_df.join(inventory_df, "product_id", "right") \
                       .select(
                           inventory_df.product_id,
                           coalesce(products_df.product_name, lit("Unknown Product")).alias("product_name"),
...
def exercise_4d(spark: SparkSession, online_sales: List[tuple], store_sales: List[tuple]):
    """Perform full outer join to combine online and store sales."""
    # Create DataFrames
    online_df = spark.createDataFrame(online_sales, ["product_id", "online_sales", "online_units"])
    store_df = spark.createDataFrame(store_sales, ["product_id", "store_sales", "store_units"])

    # Full outer join
    result = online_df.join(store_df, "product_id", "full_outer") \
                     .select(
...
def exercise_4e(spark: SparkSession, users_data: List[tuple], profiles_data: List[tuple], activities_data: List[tuple]):
    """Perform multiple joins across three tables."""
    # Create DataFrames
    users_df = spark.createDataFrame(users_data, ["user_id", "username", "email"])
    profiles_df = spark.createDataFrame(profiles_data, ["user_id", "age", "location", "subscription_type"])
    activities_df = spark.createDataFrame(activities_data, ["user_id", "activity_date", "activity_type", "duration"])

    # Multiple joins
    result = users_df.join(profiles_df, "user_id", "left") \
                    .join(activities_df.groupBy("user_id")
                         .agg(
...
def exercise_4f(spark: SparkSession, current_data: List[tuple], new_data: List[tuple]):
    ...
    # Create DataFrames
    current_df = spark.createDataFrame(current_data, ["id", "name", "value", "last_updated"])
    ...
    updates = current_df.join(new_df, "id", "inner") \
                       .select(
                           new_df.id,
                           new_df.name,
                           new_df.value,
    ...
    result = updates.union(inserts).union(unchanged) \
                   .orderBy("id")
...
def run_solutions():
    ...
    result_4f.show()
    print(f"Updates: {updates.count()}, Inserts: {inserts.count()}, Unchanged: {unchanged.count()}")
