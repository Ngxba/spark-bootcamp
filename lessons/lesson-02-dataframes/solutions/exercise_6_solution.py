...
def setup_spark() -> SparkSession:
    ...
    return (
        SparkSession.builder.appName("Exercise6-Solutions")
        .master("local[*]")
        .getOrCreate()
    )
...
def exercise_6a(spark: SparkSession, sales_data: List[tuple]):
    ...
    df = spark.createDataFrame(
        sales_data, ["product_category", "region", "sales_amount", "quantity"]
    )
...
def exercise_6b(spark: SparkSession, employee_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        employee_data, ["name", "department", "salary", "years_experience"]
    )
    ...
    result = (
        df.withColumn("salary_rank_overall", rank().over(window_overall))
...
def exercise_6c(spark: SparkSession, sales_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        sales_data, ["month", "region", "product", "sales_amount"]
    )

    # Create pivot table
    pivot_result = df.groupBy("month").pivot("region").sum("sales_amount").fillna(0)

    # Add total_sales column (sum across all regions)
    region_columns = [
        col_name for col_name in pivot_result.columns if col_name != "month"
    ]
    total_expr = sum([col(region_col) for region_col in region_columns])

    result = pivot_result.withColumn("total_sales", total_expr).orderBy("month")
...
def exercise_6d(spark: SparkSession, customer_orders: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        customer_orders, ["customer_id", "order_date", "order_amount"]
    )
    ...
    result = (
        df.withColumn("order_number", row_number().over(customer_window))
        .withColumn(
            "first_order_date", first("order_date").over(customer_window_unbounded)
        )
        .withColumn(
        ...
        .withColumn(
            "previous_order_amount", lag("order_amount", 1).over(customer_window)
        )
...
def exercise_6e(spark: SparkSession, product_reviews: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        product_reviews, ["product_id", "rating", "review_text", "helpful_votes"]
    )
    ...
    result = (
        ...
        .agg(
            ...
            spark_sum(when(col("rating") <= 2, 1).otherwise(0)).alias(
                "low_rating_count"
            ),
            spark_sum(when(col("rating") >= 4, 1).otherwise(0)).alias(
                "high_rating_count"
            ),
        )
...
def exercise_6f(spark: SparkSession, time_series_data: List[tuple]):
    ...
    result = (
        df.withColumn(
            "moving_avg_3", spark_round(avg("metric_value").over(window_3day), 2)
        )
        .withColumn(
            "moving_avg_7", spark_round(avg("metric_value").over(window_7day), 2)
        )
        .withColumn(
            "value_vs_prev",
