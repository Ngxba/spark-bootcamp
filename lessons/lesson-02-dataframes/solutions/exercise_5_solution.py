...
from pyspark.sql.functions import size, split, to_date, trim, upper, when, year
...
def setup_spark() -> SparkSession:
    ...
    return (
        SparkSession.builder.appName("Exercise5-Solutions")
        .master("local[*]")
        .getOrCreate()
    )
...
def exercise_5b(spark: SparkSession, sales_data: List[tuple]):
    ...
    df = spark.createDataFrame(
        sales_data, ["product", "quantity", "unit_price", "discount_rate"]
    )
    ...
    result = (
        ...
        .withColumn(
            "net_amount",
            spark_round(
                col("quantity") * col("unit_price") * (1 - col("discount_rate")), 2
            ),
        )
        .withColumn(
            "price_category",
...
def exercise_5c(spark: SparkSession, events_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        events_data, ["event_name", "event_date_str", "duration_days"]
    )

    # Apply date transformations
    result = (
        df.withColumn("event_date", to_date(col("event_date_str"), "yyyy-MM-dd"))
...
def exercise_5d(spark: SparkSession, products_data: List[tuple]):
    ...
    # Create DataFrame
    df = spark.createDataFrame(
        products_data, ["product_name", "tags_str", "ratings_str"]
    )
    ...
    result = (
        ...
        .withColumn(
            "ratings_array", split(col("ratings_str"), ",").cast("array<double>")
        )
        ...
        .withColumn(
            ...
            spark_round(
                expr(
                    "aggregate(split(ratings_str, ','), 0.0, (acc, x) -> acc + cast(x as double)) / size(split(ratings_str, ','))"
                ),
                1,
            ),
        )
        ...
        .withColumn(
            "has_mobile_tag", array_contains(split(col("tags_str"), ","), "mobile")
        )
    )
    ...
    result = (
        ...
        .withColumn(
            ...
            spark_round(
                expr(
                    "aggregate(transform(split(ratings_str, ','), x -> cast(x as double)), 0.0, (acc, x) -> acc + x) / size(split(ratings_str, ','))"
                ),
                1,
            ),
        )
...
def exercise_5e(spark: SparkSession, customers_data: List[tuple]):
    ...
    df = spark.createDataFrame(
        customers_data,
        ["customer_id", "age", "income", "purchase_count", "last_purchase_days"],
    )
...
def exercise_5f(spark: SparkSession, orders_data: List[tuple]):
    ...
    df = spark.createDataFrame(
        orders_data,
        ["order_id", "customer_email", "order_total_str", "order_date_str", "status"],
    )

    # Complete data processing pipeline
    result = (
        df.withColumn(
            "customer_domain", regexp_extract(col("customer_email"), r"@(.+)", 1)
        )
